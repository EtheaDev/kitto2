...
type
  ...
  public
    ...
    function FindFieldByModelFieldName(const AModelFieldName: string): TKViewField;
    ...
    function GetAliasedNames(const AFieldNames: TStringDynArray): TStringDynArray;
...
function TKViewTable.FindFieldByModelFieldName(const AModelFieldName: string): TKViewField;
begin
  Result := GetFields.FindChildByPredicate(
    function(const ANode: TEFNode): Boolean
    begin
      Result := SameText(TKViewField(ANode).ModelField.FieldName, AModelFieldName);
    end) as TKViewField;
end;
...
function TKViewTable.GetAliasedNames(const AFieldNames: TStringDynArray): TStringDynArray;
var
  I: Integer;
  LViewField: TKViewField;
begin
  ...
  begin
    LViewField := FindFieldByModelFieldName(AFieldNames[I]);
    if Assigned(LViewField) then
    begin
      if LViewField.IsReference then
        { TODO : support multi-field references }
        Result[I] := LViewField.ModelField.Fields[0].DBColumnName
      else
...
begin
  Result := GetAliasedNames(Model.GetKeyFieldNames);
end;
...
begin
  LParent := Parent;
  while (LParent is TEFNode) and not (LParent is TKViewFields) do
    LParent := TEFNode(LParent).Parent;
  Result := (LParent as TKViewFields).Table;
end;
