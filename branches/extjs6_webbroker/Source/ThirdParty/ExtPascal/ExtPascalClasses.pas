...
{$M+}
  ...
  protected
    ...
    procedure DownloadBuffer(const FileName: string; const Size: Longint; const Buffer: AnsiString;
      AContentType: string = '');
  ...
  public
    ...
    // HTTP response content-type header, default is 'text/html'
    ...
    // Max size of upload file. Default is MaxLongint(2GB)
    ...
    // Returns HTTP query info parameters read in the current request as a string
    ...
    property RequestHeader[const Name: string]: string read GetRequestHeader;
    // Returns HTTP headers read in the current request
...
{$M-}
  ...
  public
    constructor Create(const AOwner: TComponent; const ATitle: string; ASessionClass: TCustomWebSessionClass;
      APort: Word; AMaxIdleMinutes: Word = 30; AMaxConns: Integer = 1000); reintroduce; virtual;
    ...
    property OwnerThread: TThread read FOwnerThread;
    // Application will terminate if OwnerThread is not nil and OwnerThread.Terminated is True
    property Password: string read FPassword write FPassword;
    // Password to be informed in Browser URL, as a query parameter, to Shutdown and Reconfig methods
...
const
  CBrowserNames: array [TBrowser] of string = ('Unknown', 'MSIE', 'Firefox', 'Chrome', 'Safari', 'Opera', 'Konqueror',
    'Safari');
  // todo: put this map into a config file
CMIMEExtensions:
array [1 .. 176] of record Ext: string;
MimeType:
string;
end
...
procedure TCustomWebSession.AfterHandleRequest;
begin
end;
...
begin
  for LBrowser := TBrowser(Ord(brUnknown) + 1) to High(TBrowser) do
    if Pos(CBrowserNames[LBrowser], UserAgent) <> 0 then
    begin
      FBrowser := LBrowser;
      if (FBrowser = brSafari) and // Which Safari?
        (Pos('Mobile', UserAgent) > 0) and (Pos('Apple', UserAgent) > 0) then
...
procedure TCustomWebSession.DoLogout;
begin
end;

// send cookie to response
procedure TCustomWebSession.DoSetCookie(const Name, ValueRaw: string);
begin
end;

procedure TCustomWebSession.DownloadBuffer(const FileName: string; const Size: Longint; const Buffer: AnsiString;
  AContentType: string = '');
begin
...
procedure TCustomWebSession.DownloadStream(const Stream: TStream; const FileName: string; AContentType: string);
var
  Buffer: AnsiString;
  Size: Longint;
begin
  ...
  if BeforeHandleRequest then
    try
      if PathInfo = NameSpace then
        Home
      else if CanHandleUrlPath and not HandleUrlPath and not TryToServeFile then
        OnNotFoundError;
    except
      on E: Exception do
        OnError(E.Message, PathInfo, string(URLDecode(string(ARequest))));
    end;
  if CanCallAfterHandleRequest then
    AfterHandleRequest;
end;
...
var
  ...
  LPos: Integer;
begin
  LHandlerObject := GetUrlHandlerObject;
  ...
  LMethod.Code := LHandlerObject.MethodAddress(LPathInfo);
...
procedure TCustomWebSession.InitDefaultValues;
begin
end;
...
function TCustomWebSession.MethodURI(Method: TJSProcedure): string;
begin
...
begin
  Alert(Format('Method: ''%s'' not found', [PathInfo]));
end;
...
procedure TCustomWebSession.SetCookie(const Name, Value: string; Expires: TDateTime; const Domain, Path: string;
  Secure: Boolean);
var
  ValueRaw: string;
begin
...
begin
  FScriptName := Value;
  if (FScriptName = '') or (FScriptName[Length(FScriptName)] <> '/') then
...
begin
  ...
  case BlockType of
    ubtBegin:
      begin
        if UploadNeedUnknownBlock then
          Rewrite(F, 1);
        ...
        begin // unique block
          if not UploadNeedUnknownBlock then
            Rewrite(F, 1);
          Tam := J - I - 2;
          UploadResponse(True);
        end;
  ...
  if ((BlockType <> ubtUnknown) or UploadNeedUnknownBlock) and ((FileSize(F) + Tam) <= MaxUploadSize) and not FUploadedFileTooBig
  then
...
constructor TCustomWebApplication.Create(const AOwner: TComponent; const ATitle: string;
  ASessionClass: TCustomWebSessionClass; APort, AMaxIdleMinutes: Word; AMaxConns: Integer);
begin
