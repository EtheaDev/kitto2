...
threadvar
  _CurrentWebSession : TIdWebSession; // current Indy web session object
...
procedure TIdWebHTTPSession.HandleRequest(ARequest : TIdHTTPRequestInfo; AResponse : TIdHTTPResponseInfo); begin
  ...
  if FSession.SessionCookie = '' then
    FSession.SessionCookie := SessionID;
...
constructor TIdWebApplication.Create(ATitle : string; ASessionClass : TCustomWebSessionClass;
  ...
  inherited Create(nil, ATitle, ASessionClass, APort, AMaxIdleMinutes, AMaxConns);
...
procedure TIdWebApplication.TerminateAllThreads;
begin
...
procedure TIdWebSession.SendResponse(const Msg : AnsiString); begin
